...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': "/content/YOLO7/sn-trackeval/",  # Location of GT data
            'TRACKERS_FOLDER': "/content/YOLO7/sn-trackeval/tracker",  # Trackers location
            ...
            'SEQMAP_FILE': "/content/YOLO7/sn-trackeval/seqmap.txt",  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    seq = row[0]
                    seq_list.append(seq)
                    ini_file = os.path.join(self.gt_fol, 'seqinfo.ini')
                    print('INIIIIIIIIIIIIIIIII file ', ini_file)
                    if not os.path.isfile(ini_file):
                        raise TrackEvalException('ini file does not exist: ' + seq + '/' + os.path.basename(ini_file))
                    ini_data = configparser.ConfigParser()
                    ini_data.read(ini_file)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            ...
            print('not in ziiiiiiiiiiiiiiiiiiiped')
            zip_file = None
            if is_gt:
                file = self.config["GT_LOC_FORMAT"].format(gt_folder=self.gt_fol, seq=seq)
                print('elllllllllllllllllllll file', file)
            else:
                file = os.path.join(self.tracker_fol, seq + '.txt')
                print('ellllllllllll tracker fooooooooo ', file)
